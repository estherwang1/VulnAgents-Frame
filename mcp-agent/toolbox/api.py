...
class ToolboxAPI:
    ...
    def list_tools(self, category: str = None, tag: str = None, verbose: bool = False) -> dict:
        """
        列出可用工具
        ...
        Args:
            category: 按分类过滤
            tag: 按标签过滤
            verbose: 是否返回详细信息（默认 False，只返回简要列表）
        ...
        if not verbose:
            simple_list = []
            for tool_id, info in sorted(all_tools.items()):
                simple_list.append(f"{tool_id} - {info.get('description', '')}")
            return {
                "success": True,
                "tools": simple_list,
                "count": len(simple_list),
                "hint": "使用 get_tool_help(tool_id) 查看详细参数"
            }
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("  python api.py list-full              # 详细列表")
    ...
    if cmd == "list":
        ...
        result = api.list_tools(category=category, verbose=False)
        print(json.dumps(result, ensure_ascii=False, indent=2))
    
    elif cmd == "list-full":
        category = sys.argv[2] if len(sys.argv) > 2 else None
        result = api.list_tools(category=category, verbose=True)
        print(json.dumps(result, ensure_ascii=False, indent=2))
